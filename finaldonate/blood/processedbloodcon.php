...
//add record
 ...
 $serial = $_POST['serial'];
  ...
  $sql = "INSERT INTO processedblood(serial,type,status,edate,transfusionStatus,tdate) VALUES ('$serial','$type','$status','$edate','$tStatus','$tdate')";

	  if(empty($serial)  && empty($edate) && empty($type)){
	   $message= 'Please insert all required details'.  mysqli_error($con);
     }elseif(serial_exists($con ,$serial)){
        $message="blood serial entered exists in the database";
          }
...
$serial = $_POST['serial'];
 ...
 $serial =$row['serial'];
  $edate =$row['edate'];
   ...
   else{$message= 'Blood serial entered does not exist in the database'.  mysqli_error($con);} 
   }
   
   else{$message= 'Failed to select details from database'.  mysqli_error($con);}
   }
  else{$message= 'Please insert blood serial'.  mysqli_error($con);}
     }
 ...
 $serial = $_POST['serial'];
  ...
  $tdate="0000-00-00";
   ...
   if( !empty($edate) && !empty($serial)&& !empty($status) && !empty($type)){
 $query= "UPDATE processedblood SET edate='$edate', type='$type', status='$status' WHERE serial='$serial' ";
if ($query_run= mysqli_query($con,$query)){
