   ...
   $hospital = $_POST['hospital'];
    $ID = $_POST['ID'];
	  if(!empty($ID)&& !empty($serial)  && !empty($ddate) && !empty($hospital)){
	   ...
	   $sql = "INSERT INTO donatedblood(idNumber,serial,hospital,ddate) VALUES ('$ID','$serial','$hospital','$ddate')";
	   ...
	   $message= "Error:" . $sql. "<br>" . $con->error;
	    ...
	    else{$message= 'Please insert all required details'.  mysqli_error($con);}
	   }
...
$serial = $_POST['serial'];
 ...
 $serial =$row['serial'];
  ...
  $ddate =$row['ddate'];
   ...
   else{$message= 'Blood serial entered does not exist the database'.  mysqli_error($con);} 
   }
 ...
 $serial =$_POST['serial'];
  ...
  $ddate =$_POST['ddate'];
    ...
    if(!empty($ID) && !empty($ddate) && !empty($serial)  && !empty($hospital)){
 $query= "UPDATE donatedblood SET idNumber='$ID', serial='$serial',hospital='$hospital', ddate='$ddate' WHERE serial='$serial' ";
if ($query_run= mysqli_query($con,$query)){
